...
# Fig4a
{
  ...
  x2 = ggplot(df, aes(Cell.Type.L2,`Neuron Signal`,fill = Cell.Type.L2)) + 
    geom_violin(alpha=0.7)+
    geom_boxplot(width=0.25) + 
    scale_fill_manual(values = Cell.Color.L2) + 
    yuansh_theme + NoLegend()
...
#fig4c
{
  plt.list = mclapply(unique(gs$Sample),function(sample.ids){
    p1 = plot_density(gs[,gs$Sample == sample.ids],
                      features = c( "Fatty acid metabolism",'Neuron Signal'),
...
HD_Coexpression = function(obj = NULL,ids=0.99,optimal_k=3){
  quantile = quantile(sce$`Neuron Signal`, probs = ids)
  sce$`Neuron Signal` <- ifelse(sce$`Neuron Signal` > quantile, quantile, sce$`Neuron Signal`)
  ...
  df = FetchData(sce,c('Cell.Type.L1','min_distances',
                       "Angiogenesis","Neuron Signal","Glycolysis",
                       "Fatty acid metabolism",'HYPOXIA'))
  df.drop = df[,-1]
  df_melted <- melt(df.drop, id.vars = "min_distances")
  
  mydf = df_melted[df_melted$variable == 'Neuron Signal',]
  bin_size = 100
  bin_stats <- df_melted %>%
    group_by(variable) %>%
  ...
  bin_stats = bin_stats[bin_stats$variable =='Fatty acid metabolism',]
  x = ggplot(bin_stats,
...
#fig4f
sce =sce.list[['Canc']]
degs.canc = FindMarkers(sce,ident.1 = 'MSTCs')
library(msigdbr)
specific_pw <- msigdbr(species = "Homo sapiens") %>%
  filter(grepl("ANGIOGENESIS", gs_name) |
           grepl("MHC_CLASS_I", gs_name) |
           grepl("ANDRO|ESTROGEN", gs_name) |
           grepl("HYPOXIA", gs_name))

specific_pw_pathways <- split(specific_pw$gene_symbol, specific_pw$gs_name)
library(fgsea)
degs = degs.canc
...
x1 = plotEnrichment(specific_pw_pathways[["GOCC_MHC_CLASS_I_PROTEIN_COMPLEX"]], 
                    ranked_genes) + 
  labs(title = " GOCC_MHC_CLASS_I_PROTEIN_COMPLEX")
x2 = plotEnrichment(specific_pw_pathways[["GOCC_MHC_CLASS_II_PROTEIN_COMPLEX"]], 
                    ranked_genes) + 
  labs(title = " GOCC_MHC_CLASS_II_PROTEIN_COMPLEX")
x3 = plotEnrichment(specific_pw_pathways[["MANALO_HYPOXIA_UP"]], 
                    ranked_genes) + 
  labs(title = "MANALO_HYPOXIA_UP")
x4 = plotEnrichment(specific_pw_pathways[["HELLEBREKERS_SILENCED_DURING_TUMOR_ANGIOGENESIS"]], 
                    ranked_genes) + 
...
gene.list <- specific_pw_pathways[pathways]
pathways <- mysplit('GOCC_MHC_CLASS_I_PROTEIN_COMPLEX,
                    GOCC_MHC_CLASS_II_PROTEIN_COMPLEX,MANALO_HYPOXIA_UP,
                    HELLEBREKERS_SILENCED_DURING_TUMOR_ANGIOGENESIS')
gene.list <- specific_pw_pathways[pathways]
