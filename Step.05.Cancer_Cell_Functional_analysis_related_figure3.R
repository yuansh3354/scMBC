...
{
  MSTCs <- rownames(obj.Canc@meta.data)[obj.Canc$Cell.Type.L2 == "MSTCs"]
  other_cells <- rownames(obj.Canc@meta.data)[obj.Canc$Cell.Type.L2 != "MSTCs"]
  
  regulonAUC <- Cancer_Cells$regulonAUC@assays@data$AUC
  
  mstc_auc <- regulonAUC[, intersect(MSTCs, colnames(regulonAUC))]
  other_auc <- regulonAUC[, intersect(other_cells, colnames(regulonAUC))]

  mstc_mean <- rowMeans(mstc_auc)
  ...
  tf_comparison <- data.frame(
    ...
    Fold_Change = mstc_mean / (other_mean + 1e-6),
    Difference = mstc_mean - other_mean
  ) %>%
    filter(!is.na(mstc_Mean) & !is.na(Other_Mean) & 
             !is.infinite(Fold_Change) & !is.na(Fold_Change)) %>%
    mutate(
      Log2FC = log2(Fold_Change),
      Unique_to_MSTCs = Difference > quantile(Difference, 0.75, na.rm = TRUE) & 
  ...
  p1 = ggplot(tf_comparison, aes(x = Log2FC, y = Difference, color = Unique_to_MSTCs)) +
...
module_TF <- CSI_matrix_cal(regulon =t(mstc_auc),
                            CSI_threshold = 0.5,
                            module_k = 3,
                            legend_parm = "number",
                            rect_color="red",
...
{
  {
    ...
    df <- df %>%
      mutate(Cell.Type.L2 = fct_reorder(Cell.Type.L2, 
                                        M2_Neu1, .fun = median))
    p1 <- ggplot(df, aes(Cell.Type.L2,M2_Neu1,fill=Cell.Type.L2)) +
      geom_violin()+
      geom_boxplot(width = 0.25) +
      scale_fill_manual(values = Cell.Color.L2) +
  ...
  ids = c(M2_Neu,M2_immune,M2_metabolism,M2_other)
  avg_expr_scaled = avg_expr_scaled[ids,]
  gene_anno <- data.frame(
    Gene = rownames(avg_expr_scaled),
    Module = case_when(
      rownames(avg_expr_scaled) %in% M2_Neu1 ~ "Neuronal",
